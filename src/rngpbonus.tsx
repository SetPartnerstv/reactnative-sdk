...
const RNGPBonus = forwardRef<GPBonusHandle, GPBonusProps & WebViewProps>(
  (
    {
      widgetUrl = defaultWidgetUlr,
      token = '',
    ...
    const [webNavigationState, setWebNavigationState] = useState<
      WebViewNavigation | {url: string; canGoBack: boolean}
    >({url: '', canGoBack: false});
    ...
    const onMobileEvent = useCallback(
      (event: {nativeEvent: {data: string}}) => {
        const mobileEvent = JSON.parse(event?.nativeEvent?.data);
        if (printDebugInfo) {
          console.log(
            `Got Event from widget: ${JSON.stringify(
        ...
        switch (mobileEvent?.type) {
          case MobileEventType.CLIPBOARDWRITE:
            if (printDebugInfo) {
              console.log(
                `${MobileEventType.CLIPBOARDWRITE} text: ${mobileEvent?.clipboard_write}`,
              );
            }
          ...
          case MobileEventType.SHAREURLREQUEST:
            if (printDebugInfo) {
              console.log(
                `${MobileEventType.SHAREURLREQUEST} link: ${mobileEvent?.share_url_request}`,
              );
            }
            Share.share({
          ...
          case MobileEventType.OPENURLREQUEST:
            if (printDebugInfo) {
              console.log(
                `${MobileEventType.OPENURLREQUEST} link: ${mobileEvent?.open_url_request}`,
              );
            }
            ...
            //     </intent>
            // </queries>
            Linking.canOpenURL(mobileEvent?.open_url_request).then(
              supported => {
                if (!checkExternalUrl || supported) {
                  ...
                  if (printDebugInfo) {
                    console.log(
                      "Can't open URI: " + mobileEvent?.open_url_request,
                    );
                  }
                }
                return false;
    ...
    const isWidgetUrl = useCallback(
      (url: string): boolean => {
        const getDomainName = (currentUrl: string): string => {
          if (currentUrl) {
    ...
    return (
      <View style={styles.container}>
        {!!widgetUlrWithParams && (
          <WebView
            ref={webview}
            source={{uri: widgetUlrWithParams}}
            style={styles.content}
            onNavigationStateChange={handleWebViewNavigationStateChange}
            onMessage={onMobileEvent}
            injectedJavaScriptBeforeContentLoaded={widgetScript}
